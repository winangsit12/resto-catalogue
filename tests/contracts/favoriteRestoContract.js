const itActsAsFavoriteRestoModel = (favoriteResto) => {
  ...
  it('should be able to search for restos', async () => {
    favoriteResto.putResto({ id: 1, title: 'menu a' });
    favoriteResto.putResto({ id: 2, title: 'menu b' });
    favoriteResto.putResto({ id: 3, title: 'menu abc' });
    favoriteResto.putResto({ id: 4, title: 'ini mah menu abcd' });

    expect(await favoriteResto.searchRestos('menu a')).toEqual([
      { id: 1, title: 'menu a' },
      { id: 3, title: 'menu abc' },
      { id: 4, title: 'ini mah menu abcd' },
    ]);
  });
};
